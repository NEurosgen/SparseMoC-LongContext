...
def fused_kernel(
    ...
    for k_start in range(0, K, BLOCK_K):
        ...
        tl.store(u_out_ptr, u_val, mask=mask_k)
...
def fused_backward_kernel(
    ...
    for k_start in range(0, K, BLOCK_K):
        ...
        grad_z_ptrs = grad_z_act_ptr + pid_m * stride_gzm + stride_gzk * offs_k
        g_act_ptrs = g_act_ptr + pid_m * stride_gm + stride_gk * offs_k
        u_act_ptrs = u_act_ptr + pid_m * stride_um + stride_uk * offs_k

        indeces = tl.load(idx_pttrs, mask=mask_k, other=0)
        grad_z = tl.load(grad_z_ptrs, mask=mask_k, other=0)
        g_val = tl.load(g_act_ptrs, mask=mask_k, other=0)
        u_val = tl.load(u_act_ptrs, mask=mask_k, other=0)

        g_f32 = g_val.to(tl.float32)
        u_f32 = u_val.to(tl.float32)
        gz_f32 = grad_z.to(tl.float32)

        sig_g = tl.sigmoid(g_f32)
        silu_g = sig_g * g_f32

        grad_silu_g = sig_g + silu_g * (1.0 - sig_g)

        grad_u_active = gz_f32 * silu_g
        grad_g_active = gz_f32 * u_f32 * grad_silu_g
